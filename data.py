...
def collate_pool(dataset_list):
	batch_atom_fea, batch_nbr_fea, batch_nbr_fea_idx,batch_adj,batch_group_num,batch_periodic_num = [], [], [],[],[],[]
	crystal_atom_idx, batch_targets = [], []
	batch_cif_ids = []
	batch_formula=[]
	base_idx = 0
	for i, ((atom_fea, nbr_fea, nbr_fea_idx,adj,group_num,periodic_num), targets, cif_id,formula) in enumerate(dataset_list):
		if targets != float("inf"):
			n_i = atom_fea.shape[0]  # number of atoms for this crystal
			batch_adj.append(adj)
			batch_group_num.append(group_num)
			batch_periodic_num.append(periodic_num)
...
class CIFData(Dataset):
	...
	id_prop.csv: a CSV file with multiple columns. The first column recodes a
	unique ID for each crystal, and the second onward columns record the value of
	...
	targets: torch.Tensor shape (n_p, )
	cif_id: str or int
	"""
	...
	def __getitem__(self, idx):
		...
		group_num=atom_fea[:,0:18]
		periodic_num = atom_fea[:, 18:27]
		...
		periodic_num = torch.Tensor(periodic_num)
		...
		return (atom_fea, nbr_fea, nbr_fea_idx,adj,group_num,periodic_num),targets, cif_id,crystal.formula
